use belgie::{
    runtime::DenoExecutionHandle, types::runner::RunnerArguments, utils::cancel_guard::CancelGuard,
};
use pyo3::{Py, PyAny, PyResult, Python};

use crate::{convert::js_value_to_py, py_error};

type SyncRunnerResult = PyResult<Py<PyAny>>;
type AsyncRunnerResult = PyResult<Py<PyAny>>;
...
pub(crate) async fn execute_async(
    ...
    result
        .map_err(py_error::from_binding_error)
        .and_then(|value| Python::attach(|py| js_value_to_py(&value, py)))
}

#[cfg(test)]
mod tests {
    use super::{execute_async, execute_sync};
    use crate::convert::runner_arguments_from_py;
    use belgie::{
    ...
    fn empty_arguments() -> RunnerArguments {
        with_python(|py| {
            ...
            runner_arguments_from_py(&args, None).expect("empty args should convert")
        })
    }
    ...
    fn scalar_input_arguments() -> RunnerArguments {
        with_python(|py| {
            ...
            runner_arguments_from_py(&args, None).expect("input args should convert")
        })
    }
    ...
    fn input_arguments() -> RunnerArguments {
        with_python(|py| {
            ...
            runner_arguments_from_py(&args, Some(&kwargs)).expect("input args should convert")
        })
    }
