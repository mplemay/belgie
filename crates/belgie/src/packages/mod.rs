...
pub const EMPTY_DENO_LOCK: &str = "{\"version\":\"5\"}\n";
pub const BELGIE_DIR: &str = ".belgie";
...
impl DependencyLayout {
    pub fn from_dependencies(dependencies: &[PackageDependency]) -> Self {
...
impl PackageDependency {
    ...
    pub fn registry_specifier(&self) -> Option<&str> {
        match &self.kind {
    ...
    pub fn set_registry_specifier(&mut self, specifier: String) {
...
#[derive(Clone, Debug)]
pub struct EnvironmentInstallResult {
    pub lockfile: PathBuf,
    pub dependencies: usize,
}

#[derive(Clone, Debug)]
pub struct EnvironmentUpdateResult {
    pub lockfile: PathBuf,
...
#[derive(Clone, Debug)]
pub struct EnvironmentUpdateChange {
    pub name: String,
    pub previous: String,
    pub updated: String,
}
...
pub fn dependencies_from_mapping(
    workspace: &Path,
...
pub fn is_registry_import_specifier(specifier: &str) -> bool {
    specifier.starts_with("npm:") || specifier.starts_with("jsr:")
}

pub fn is_resolver_import_specifier(specifier: &str) -> bool {
    ModuleSpecifier::parse(specifier)
        .ok()
        .is_some_and(|url| matches!(url.scheme(), "npm" | "jsr" | "http" | "https"))
}

pub fn import_map_value(dependencies: &[PackageDependency]) -> Result<serde_json::Value, AnyError> {
    Ok(serde_json::json!({
        "imports": dependency_imports(dependencies)?,
    }))
}

pub fn specified_import_map(
    base_url: Url,
...
pub fn refresh_specified_import_map(
...
pub fn local_file_dependency_install_roots(
    dependencies: &[PackageDependency],
...
pub fn has_legacy_local_file_state(install_root: &Path) -> bool {
    install_root.join(BELGIE_DIR).is_dir()
}

pub fn sync_local_file_dependencies(
    install_root: &Path,
...
pub async fn install_environment_packages(
    cwd: PathBuf,
    lockfile: PathBuf,
    lockfile_only: bool,
...
pub async fn update_environment_packages(
