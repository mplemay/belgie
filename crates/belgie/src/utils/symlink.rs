...
pub fn remove_symlink(path: &Path) -> Result<(), AnyError> {
...
pub fn remove_symlink_if_present(path: &Path) -> Result<(), AnyError> {
...
pub fn create_directory_symlink(target: &Path, link: &Path) -> Result<(), AnyError> {
    let context = format!(
