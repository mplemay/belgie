...
pub fn materialize_node_modules(
    cwd: &Path,
...
pub fn cleanup_materialized(path: &Path, expected_target: &Path) -> Result<(), AnyError> {
...
#[cfg(test)]
pub fn create_dangling_dir_symlink(link: &Path, target: &Path) {
    #[cfg(unix)]
