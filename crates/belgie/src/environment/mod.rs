...
impl EnvironmentDefinition {
    ...
    pub fn from_mapping_with_options(
        workspace: PathBuf,
        persist_path: Option<PathBuf>,
        dependencies: std::collections::BTreeMap<String, String>,
        lockfile_source: Option<PathBuf>,
...
impl SharedEnvironment {
    pub fn new(definition: EnvironmentDefinition) -> Self {
        Self {
    ...
    pub fn activate_blocking(&self) -> Result<Arc<ActiveEnvironment>, AnyError> {
    ...
    pub fn acquire_active(&self) -> Result<Arc<ActiveEnvironment>, AnyError> {
        let state = self
            .state
            .lock()
            .expect("environment state lock should not be poisoned");
    ...
    pub fn deactivate(&self) -> Result<(), AnyError> {
        let mut state = self
            .state
            .lock()
            .expect("environment state lock should not be poisoned");
    ...
    pub async fn install(&self) -> Result<EnvironmentInstallResult, AnyError> {
    ...
    pub fn install_blocking(&self) -> Result<EnvironmentInstallResult, AnyError> {
        let environment = self.clone();
...
impl ActiveEnvironment {
    ...
    pub fn refresh_local_file_dependencies(&self, already_synced: bool) -> Result<(), AnyError> {
    ...
    pub fn cleanup_materialized_node_modules(&self) -> Result<(), AnyError> {
        let EnvironmentRoot::Ephemeral {
            temp_dir,
            materialized_node_modules,
        } = &self.root
    ...
    pub fn embed_context_with_worker_options(
        &self,
    ...
    pub fn uses_package_loader(&self) -> bool {
    ...
    pub fn needs_package_environment(&self, worker_options: &RuntimeWorkerOptions) -> bool {
