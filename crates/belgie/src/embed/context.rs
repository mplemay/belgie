...
impl EmbedHttpClient {
    ...
    pub async fn fetch_bytes(&self, url: &Url) -> Result<Vec<u8>, JsErrorBox> {
...
#[derive(Clone, Debug)]
pub struct EmbedContextOptions {
...
impl EmbedContextOptions {
    pub fn for_package_manager(mut self) -> Self {
        self.is_package_manager_subcommand = true;
        self
    }

    pub fn requires_embed_module_loader(&self) -> bool {
        !self.allow_remote
    ...
    pub fn apply_worker_options(&mut self, worker: &RuntimeWorkerOptions) {
...
impl EmbedContext {
    ...
    pub fn module_read_checker(&self) -> &ModuleReadChecker {
