...
pub struct CommandExecutionOptions {
    pub package_environment: BoundPackageEnvironment,
    pub js_runtime_options: JsRuntimeOptions,
    pub runtime_worker_options: RuntimeWorkerOptions,
    pub runtime_root: PathBuf,
    pub command: CommandSource,
    pub argv: Vec<String>,
    pub worker_factory_roots: LibWorkerFactoryRoots,
}

impl CommandExecutionHandle {
    pub fn spawn(options: CommandExecutionOptions) -> Self {
    ...
    pub fn wait_blocking(&self) -> CommandResult {
    ...
    pub async fn wait_async(&self) -> CommandResult {
        let response = self.take_response()?;
        response
            .await
            .map_err(|_| BindingError::runtime("Command worker stopped unexpectedly"))?
    ...
    pub fn close_blocking(&self) -> CommandResult {
        self.cancel();
