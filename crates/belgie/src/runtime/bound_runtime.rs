...
#[derive(Clone, Debug)]
pub enum BoundPackageEnvironment {
    Isolated(Arc<ActiveEnvironment>),
    Implicit(Arc<ImplicitPackageEnvironment>),
}

pub struct ImplicitPackageEnvironment {
    workspace: PathBuf,
    lockfile: PathBuf,
...
impl ImplicitPackageEnvironment {
    pub fn new(workspace: &Path) -> Result<Self, BindingError> {
...
impl BoundPackageEnvironment {
    pub fn implicit_for_cwd(cwd: &Path) -> Result<Self, BindingError> {
        Ok(Self::Implicit(Arc::new(ImplicitPackageEnvironment::new(
            cwd,
        )?)))
    }

    pub fn from_isolated_runtime(runtime: &DenoRuntime) -> Result<Option<Self>, BindingError> {
    ...
    pub fn for_script_without_package_loader(existing: Option<&Self>) -> Option<Self> {
        match existing {
    ...
    pub fn refresh_local_file_dependencies(&self) -> Result<(), BindingError> {
        match self {
            Self::Isolated(environment) => environment
...
impl BoundRuntime {
    pub fn new(runtime: DenoRuntime, script: ScriptSource) -> Self {
        Self {
            runtime,
            script,
    ...
    pub fn js_runtime_options(&self) -> &JsRuntimeOptions {
        self.runtime.js_runtime_options()
    }

    pub fn worker_options(&self) -> &RuntimeWorkerOptions {
        self.runtime.worker_options()
    }

    pub fn package_environment(&self) -> Option<&BoundPackageEnvironment> {
        self.package_environment.as_ref()
    }

    pub fn with_package_environment(
        mut self,
    ...
    pub fn run_signature(&self) -> Option<&crate::script::RunSignature> {
