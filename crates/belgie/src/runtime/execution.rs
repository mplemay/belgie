...
use crate::{
    ...
    types::{error::BindingError, runner::RunnerArguments, value::JsValue},
...
#[derive(Clone, Debug)]
pub struct DenoExecutionHandle {
...
enum ExecutionCommand {
    Invoke {
        ...
        respond_to: oneshot::Sender<ExecutionResult<JsValue>>,
    },
    Shutdown,
}

impl DenoExecutionHandle {
    pub fn new(bound: BoundRuntime, worker_factory_roots: LibWorkerFactoryRoots) -> Self {
    ...
    pub fn close_blocking(&self) -> ExecutionResult<()> {
    ...
    pub fn invoke_blocking(&self, arguments: RunnerArguments) -> ExecutionResult<JsValue> {
        if self.is_closed() {
    ...
    pub async fn invoke_async(&self, arguments: RunnerArguments) -> ExecutionResult<JsValue> {
        if self.is_closed() {
            return Err(BindingError::runtime("Deno execution runner is closed"));
        }
        let (respond_to, response) = oneshot::channel();
...
impl DenoExecutionContext {
    ...
    async fn invoke(&mut self, arguments: RunnerArguments) -> ExecutionResult<JsValue> {
...
#[cfg(test)]
pub fn with_test_js_runtime<F, R>(f: F) -> R
where
