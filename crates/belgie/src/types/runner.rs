...
use crate::types::{error::BindingError, value::JsValue};

#[derive(Clone, Debug)]
pub struct RunnerArguments {
    positional: Vec<JsValue>,
...
impl RunnerArguments {
    pub fn new(positional: Vec<JsValue>, keyword: Map<String, Value>) -> Self {
        Self {
            positional,
            keyword,
        }
    ...
    fn legacy_values_for_call(&self) -> Vec<JsValue> {
        let mut values = self.positional.clone();
        if !self.keyword.is_empty() {
            values.push(JsValue::from_json(Value::Object(self.keyword.clone())));
        }
        values
    }
    ...
    fn map_to_signature(
        ...
        for (name, value) in &self.keyword {
            if let Some(index) = find_ident_param_index(&slots, signature, name, |slot| {
                ...
                assign_slot_value(&mut slots[index], JsValue::from_json(value.clone()))?;
                continue;
            }
...
fn assign_positional_args(
    ...
    while index < positional_len {
        ...
        if matches!(signature.params[index], ParamPattern::Rest(_)) {
            ...
            assign_slot_value(
                &mut slots[index],
                JsValue::from_json(Value::Array(rest_values)),
            )?;
            return Ok(());
        }
...
fn apply_single_param_keyword(
    ...
    match param {
        ParamPattern::Ident {
            ...
            if ident == name {
                assign_slot_value(slot, JsValue::from_json(value))
...
fn assign_slot_value(slot: &mut SlotState, value: JsValue) -> Result<(), BindingError> {
    match slot {
        SlotState::Empty => {
    ...
    for (index, slot) in slots[..=last_index].iter().enumerate() {
        arguments.push(match slot {
            SlotState::Empty if matches!(signature.params[index], ParamPattern::Rest(_)) => {
                CallArgument::Value(JsValue::from_json(Value::Array(vec![])))
            }
            SlotState::Empty => CallArgument::Undefined,
            SlotState::Value(value) => CallArgument::Value(value.clone()),
            SlotState::Object(object) => {
                CallArgument::Value(JsValue::from_json(Value::Object(object.clone())))
            }
        });
    }
    Ok(arguments)
            ...
            match value.into_json() {
                Value::Array(elements) => {
                    expanded.extend(
                        elements
                            .into_iter()
                            .map(|element| CallArgument::Value(JsValue::from_json(element))),
                    );
                }
                other => expanded.push(CallArgument::Value(JsValue::from_json(other))),
            }
            continue;
        }
...
mod tests {
    ...
    fn runner_arguments_from(
        ...
        RunnerArguments::new(
            positional.into_iter().map(JsValue::from_json).collect(),
            keywords,
        )
    }
    ...
    fn rejects_duplicate_keyword_for_filled_param() {
        ...
        let arguments = RunnerArguments {
            positional: vec![JsValue::from_json(Value::Number(1.into()))],
