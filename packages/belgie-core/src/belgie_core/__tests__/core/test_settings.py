...
def test_auth_settings_required_fields() -> None:
    with pytest.raises(ValidationError) as exc_info:
        BelgieSettings()  # type: ignore[call-arg]

    errors = exc_info.value.errors()  # type: ignore[attr-defined]
    assert len(errors) > 0


def test_auth_settings_rejects_empty_base_url() -> None:
    with pytest.raises(ValidationError):
        BelgieSettings(secret="test-secret", base_url="")  # type: ignore[call-arg]
...
def test_auth_settings_full_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = BelgieSettings()  # type: ignore[call-arg]
