...
class OAuthPlugin(Plugin):
    ...
    def router(self, belgie: Belgie) -> APIRouter:
        ...
        _add_metadata_route(router, metadata)
        _add_authorize_route(router, belgie, provider, self._settings)
        _add_token_route(router, provider)
        _add_login_route(router, issuer_url, self._demo_username, self._demo_password)
        _add_login_callback_route(router, belgie, provider, self._demo_username, self._demo_password)
        _add_introspect_route(router, provider)
...
def _add_metadata_route(router: APIRouter, metadata: OAuthMetadata) -> None:
...
def _add_token_route(router: APIRouter, provider: SimpleOAuthProvider) -> None:  # noqa: C901
...
def _add_login_route(router: APIRouter, issuer_url: str, demo_username: str, demo_password: str) -> None:
...
def _add_introspect_route(router: APIRouter, provider: SimpleOAuthProvider) -> None:
