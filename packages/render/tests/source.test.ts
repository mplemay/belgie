...
describe("inline source transform", () => {
  test("removes the plugins expression and imports used only by plugins", () => {
    const source = [
  ...
  test("leaves source without a server plugins property unchanged", () => {
  ...
  test("removes first and only plugin properties and whole plugin imports", () => {
  ...
  test("retains default and namespace browser imports from mixed declarations", () => {
  ...
  test("rejects malformed npm specifiers", () => {
    expect(() => normalizeNpmSpecifier("npm:@scope")).toThrow("invalid npm specifier");
  });

  test("resolves and loads the complete virtual browser module graph", async () => {
    ...
    const entryId = await resolve.call(context as never, "virtual:belgie-render/client-entry", undefined, {} as never);
    const callerId = await resolve.call(context as never, "virtual:belgie-render/caller", undefined, {} as never);
    const apiId = await resolve.call(context as never, CLIENT_RENDER_ID, undefined, {} as never);
    const packageApiId = await resolve.call(context as never, "npm:@belgie/render@0.1.0", undefined, {} as never);
    const npmId = await resolve.call(context as never, "npm:react@19.2.6", undefined, {} as never);
    const unknownId = await resolve.call(context as never, "unknown", undefined, {} as never);
