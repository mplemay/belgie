...
describe("virtual project validation", () => {
  it.each([
    ...
    "nested/../outside.ts",
    "/absolute.ts",
    String.raw`C:\absolute.ts`,
    String.raw`nested\windows.ts`,
    "widget.tsx",
    "nul\0file.ts",
...
describe("in-memory widget builds", () => {
  it("builds relative modules, JSON, CSS, and textual assets into stable HTML", async () => {
  ...
  it("does not evaluate widget source or discover user Vite configuration", async () => {
    const root = temporaryProject();
  ...
  it("enforces default exports and reports virtual filenames", async () => {
    const root = temporaryProject();
  ...
  it.each([
    ...
    "deno:fs",
    "/tmp/file.ts",
  ])("blocks untrusted import %s", async (specifier) => {
    const root = temporaryProject();
    await assert.rejects(
      buildWidget({
        root,
  ...
  it.each(["vite", "vite/plugin", "rolldown", "lightningcss", "@belgie/mcp/vite", "@belgie/mcp/builder"])(
  ...
  it("allows only explicitly supplied host package aliases", async () => {
    const root = temporaryProject();
    const widget = [
      'import { z } from "zod";',
  ...
  it("rejects malformed dependency aliases and non-text files", async () => {
    const root = temporaryProject();
    await assert.rejects(
