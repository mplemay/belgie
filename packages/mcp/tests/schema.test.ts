...
describe("schema identifiers", () => {
  test("normalizes type names and allocates collisions", () => {
  ...
  test("normalizes value names, reserved words, numbers, and collisions", () => {
...
describe("schema compilation", () => {
  test("compiles primitives, nullable types, and type unions", () => {
    assert.deepEqual(compile({ type: "string", nullable: true }), [
      "export type Example = string | null;",
    ]);
    assert.deepEqual(
      compile({ type: ["string", "number", "integer", "boolean", "null"] }),
      ["export type Example = string | number | boolean | null;"],
    );
    assert.deepEqual(compile({ type: "object" }), [
      "export type Example = Record<string, never>;",
    ]);
    assert.deepEqual(compile({ type: "array" }), [
      "export type Example = readonly unknown[];",
    ]);
  });

  test("compiles constants and enum JSON literals", () => {
    assert.deepEqual(
      compile({
        enum: [null, "ok", true, 2, ["x", false], { z: 1, a: "first" }],
      }),
      [
  ...
  test("compiles unions, intersections, and boolean schemas", () => {
    assert.deepEqual(
      compile({
        allOf: [
          { oneOf: [{ type: "string" }, { type: "number" }] },
          { anyOf: [true, { type: "boolean" }] },
        ],
      }),
      ["export type Example = string | number;"],
    );
    assert.deepEqual(compile({ allOf: [false, { type: "string" }] }), [
      "export type Example = never;",
    ]);
    assert.deepEqual(compile({ anyOf: [] }), ["export type Example = never;"]);
  });

  test("compiles objects with required, optional, and index properties", () => {
    ...
    assert.deepEqual(compile({ additionalProperties: true }), [
  ...
  test("compiles homogeneous arrays and parenthesizes compound elements", () => {
    assert.deepEqual(compile({ items: false }), [
      "export type Example = readonly never[];",
    ]);
    assert.deepEqual(compile({ items: true }), [
      "export type Example = readonly unknown[];",
    ]);
    assert.deepEqual(
      compile({ items: { oneOf: [{ type: "string" }, { type: "number" }] } }),
      ["export type Example = readonly (string | number)[];"],
    );
  });

  test("compiles prefix and legacy tuple arrays", () => {
    assert.deepEqual(
      compile({ prefixItems: [{ type: "string" }, { type: "number" }], maxItems: 2 }),
      ['export type Example = readonly [string, number];'],
    );
    assert.deepEqual(
      compile({ prefixItems: [{ type: "string" }], items: { type: "boolean" } }),
      ['export type Example = readonly [string, ...boolean[]];'],
    );
    assert.deepEqual(
      compile({ prefixItems: [{ type: "string" }], items: false }),
      ['export type Example = readonly [string];'],
    );
    assert.deepEqual(
      compile({ items: [{ type: "string" }], additionalItems: true }),
      ['export type Example = readonly [string, ...unknown[]];'],
    );
    assert.deepEqual(
      compile({ items: [{ type: "string" }], additionalItems: { type: "number" } }),
      ['export type Example = readonly [string, ...number[]];'],
    );
    assert.deepEqual(
      compile({ items: [{ type: "string" }], additionalItems: false }),
      ['export type Example = readonly [string];'],
    );
    assert.deepEqual(
      compile({ items: [{ type: "string" }], maxItems: 1 }),
      ['export type Example = readonly [string];'],
    );
  });

  test("compiles local definitions and JSON pointer references", () => {
    const declarations = compile({
      ...
      allOf: [
        ...
        { $ref: "#/$defs/a~1b~0c", type: "string", description: "sibling" },
      ],
    });
  ...
  test("keeps definitions globally collision-free", () => {
    const allocator = new IdentifierAllocator();
...
describe("schema validation", () => {
  test.each([
    ...
    [{ prefixItems: [], items: [] }, /items must be a JSON Schema object/u],
    [{ items: [], additionalItems: [] }, /additionalItems must be a JSON Schema object/u],
    [{ items: { type: "string" }, additionalItems: false }, /uses additionalItems without/u],
